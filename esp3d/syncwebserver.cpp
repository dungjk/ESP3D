...
{
  // CORS
  if (web_interface->web_server.method() == HTTP_OPTIONS)
  {
    web_interface->web_server.sendHeader("Access-Control-Allow-Origin", "*");
    web_interface->web_server.sendHeader("Access-Control-Max-Age", "10000");
    web_interface->web_server.sendHeader("Access-Control-Allow-Methods", "PUT,POST,GET,OPTIONS");
    web_interface->web_server.sendHeader("Access-Control-Allow-Headers", "*");
    web_interface->web_server.send(204);
    return;
  }
...
void SDFile_serial_upload()
{
  ...
  else
  {
    ...
    if ((web_interface->_upload_status != UPLOAD_STATUS_FAILED) || (upload.status == UPLOAD_FILE_START))
    {
      ...
      { //if com error no need to send more data to serial
        for (int pos = 0; (pos < upload.currentSize) && (web_interface->_upload_status == UPLOAD_STATUS_ONGOING); pos++)
