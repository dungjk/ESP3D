...
bool COMMAND::execute_command(int cmd, String cmd_params, tpipe output, level_authenticate_type auth_level, ESPResponseStream *espresponse)
{
  ...
  case 110:
    ...
    if (response)
    {
      ...
      { //restart device is the best way to start everything clean
        if ((WiFi.getMode() == WIFI_OFF) || !wifi_config.WiFi_on)
        {
      ...
      { //restart wifi and restart is the best way to start everything clean
        ESPCOM::println("Enabling Wifi", output, espresponse);
        web_interface->restartmodule = true;
      }
    }
  ...
  { //send GCode with check sum caching right line numbering
    ...
    { //it may failed because of skip if repetier so let's reset numbering first
